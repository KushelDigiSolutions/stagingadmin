<table class="table align-middle table-row-dashed fs-6 gy-5">
    ...
    <tbody>
        @foreach ($footers as $key => $footer)
            <tr>
                <td>{{ $footer->title }}</td>
                <td>{{ App\Models\Admin\Admin::where('id', $footer->added_by)->first()->name }}</td>
                ...
                {{-- <td>
                    <div class="d-flex flex-shrink-0">
                        <a href="{{route('admin.footers.edit',$footer->id)}}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                            <i class="ki-duotone ki-notepad-edit fs-2">
                                <span class="path1"></span>
                                <span class="path2"></span>
                            </i>
                        </a>
                        <form id="delete-form" action="{{route('admin.footers.destroy',$footer->id)}}" method="POST" onsubmit="return confirm('Are you want delete this Footer!');">
                            @method('DELETE')
                            @csrf
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-more-details menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4 mt-10 new-dp1 dropdown-menu"
                            ...
                            <div class="menu-item px-3">
                                @can('footer-edit')
                                <a href="{{route('admin.footers.edit',$footer->id)}}" class="menu-link px-3">
                                ...
                                @endcan
                                @can('footer-delete')
                                <a class="menu-link px-3" onclick="confirmDelete({{$footer->id}})">
                                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                ...
                                <form action="{{route('admin.footers.destroy',$footer->id)}}"
                                    id="delete_form_{{$footer->id}}" method="POST">
                                    @method('DELETE')
                                    @csrf
                                </form>
                                @endcan
...
<div class="row">
    <div class="col-md-4">
        <p>
            <b>Showing {{ ($footers->currentpage() - 1) * $footers->perpage() + 1 }} to
                {{ ($footers->currentpage() - 1) * $footers->perpage() + $footers->count() }} of
                {{ $footers->total() }} Footers
            </b>
        </p>
    </div>
