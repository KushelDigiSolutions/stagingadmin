                        ...
                        <div class="dropdown">
                            ...
                            <div class="dropdown-more-details menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4 mt-10 new-dp1 dropdown-menu"
                                ...
                                <div class="menu-item px-3">
                                    ...
                                    @endcan
                                    @can('role-delete')
                                        <a class="menu-link px-3" onclick="confirmDelete({{ $data->id }})">
                                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
                                        ...
                                        <form action="{{ route('admin.roles.destroy', $data->id) }}"
                                            id="delete_form_{{ $data->id }}" method="POST">
                                            @method('DELETE')
                                            @csrf
                                        </form>
                                    @endcan
...
<div class="row">
    <div class="col-md-4">
        <p>
            <b>Showing {{ ($list->currentpage() - 1) * $list->perpage() + 1 }} to
                {{ ($list->currentpage() - 1) * $list->perpage() + $list->count() }} of
                {{ $list->total() }} Roles
            </b>
        </p>
    </div>
