<table class="table align-middle table-row-dashed fs-6 gy-5">
    <thead>
        <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
            ...
            @canany(['event-categories-edit', 'event-categories-delete'])
            ...
            <tr>
                ...
                <td>
                    <div class="d-flex align-items-center">
                        <div class="symbol symbol-45px me-5">
                            <img src="{{ asset('backend/admin/images/event_management/categories/' . $category->image) }}"
                ...
                <td>
                    @if ($category->is_active == '1')
                        <a href="{{ route('admin.event-categories.show', $category->id) }}?status=0">
                            <div class="badge badge-light-success">Active</div>
                        </a>
                    @else
                        <a href="{{ route('admin.event-categories.show', $category->id) }}?status=1">
                        ...
                        <div class="dropdown">
                            ...
                            <div class="dropdown-more-details menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4 mt-10 new-dp1 dropdown-menu"
                                ...
                                <div class="menu-item px-3">
                                    @can('event-categories-edit')
                                        <a href="{{ route('admin.event-categories.edit', $category->id) }}"
                                    ...
                                    @endcan
                                    @can('event-categories-delete')
                                        <a class="menu-link px-3" onclick="confirmDelete({{ $category->id }})">
                                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
                                        ...
                                        <form action="{{ route('admin.event-categories.destroy', $category->id) }}"
...
<div class="row">
    <div class="col-md-4">
        <p>
            <b>Showing {{ ($categories->currentpage() - 1) * $categories->perpage() + 1 }} to
                {{ ($categories->currentpage() - 1) * $categories->perpage() + $categories->count() }} of
