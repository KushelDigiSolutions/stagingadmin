...
class EquipmentRentalController extends Controller
{
    ...
    public function getEquipment(Request $request , $slug = null)
    {
        ...
        $categories = EquipmentCategory::select('id', 'name', 'slug', 'image', 'seo_title', 'seo_description')->get();
 ...
 $eventscount = Equipment::where('is_delete', '0')->whereJsonContains('category_ids', '' . $cat)->count();
        ...
        return response()->json(['equipment' => $events,'count'=>$eventscount,'categories'=>$categories,'media_url'=>asset('backend/admin/images/equipment_management/equipment/'),'cat_url'=>asset('backend/admin/images/equipment_management/categories/')]);
    }
    ...
    public function searchEquipment(Request $request,$slug = null){
        ...
        $data["items"] = Equipment::where('title', 'like', "%$query%")->orWhere('description', 'like', "%$query%")->where('is_delete', '0')->orderBy('created_at', $filter)->offset($offset)->limit($limit)->get();
        $data["total_count"] = Equipment::where('title', 'like', "%$query%")->orWhere('description', 'like', "%$query%")->where('is_delete', '0')->orderBy('created_at', $filter)->offset($offset)->limit($limit)->count();
        $data["image_path"] = url("backend/admin/images/equipment_management/equipment/"); 
    ...
    public function getEquipmentCategory(){
        $categories = EquipmentCategory::select('id', 'name', 'slug', 'image', 'seo_title', 'seo_description')->where('is_delete', '0')->get(); 
     ...
     public function getEquipmentHome(Request $request , $slug = null)
    {
        ...
        foreach ($categories as $category) {
            ...
            $category->count = Equipment::where('is_delete', '0')->whereJsonContains('category_ids', '' . $category->id)->orderByDesc('date')->count();
        }
        $data['category_all'] = $categories;
        $data['items'] = Equipment::where('is_delete', '0')->whereJsonContains('category_ids', '' . $categories[0]->id)->offset($offset)->orderBy('created_at', $filter)->limit($limit)->get();
        ...
        $data["image_path"] =  asset('backend/admin/images/equipment_management/equipment/');
		$data["image_cat"] = url("backend/admin/images/equipment_management/categories/"); 
    ...
    public function getEquipmentSearch()
    {
        ...
        $data["listing_first"]["items"] = Equipment::where('is_delete', '0')->orderBy('created_at', 'desc')->limit(2)->get();
        ...
        $data["listing_second"]["items"] = Equipment::where('is_delete', '0')->orderBy('created_at', 'desc')->limit(10)->get();
    ...
    $data["listing_third"]["title"] = "We list the most popular unique rental camping around the world!";
        $data["listing_third"]["items"] = Equipment::where('is_delete', '0')->orderBy('created_at', 'desc')->limit(10)->get();
        ...
        $data["listing_fourth"]["items"] = Equipment::where('is_delete', '0')->orderBy('created_at', 'desc')->limit(10)->get();
    ...
    public function getEquipmentApp()
    {
        ...
        foreach ($events as $key => $event) {
            ...
            foreach (json_decode($images) as $yy => $image) {
                $dataArray['event'][$key]['image'][$yy] = asset('backend/admin/images/equipment_management/equipment/' . $image);
            }
        }
    ...
    public function getEquipmentBySlug($slug)
    {
        ...
        if ($event) {
            ...
            foreach (json_decode($images) as $yy => $image) {
                ...
                $dataArray['equipment']['image'][$yy] = asset('backend/admin/images/equipment_management/equipment/' . $image);
            }
        ...
        $dataArray["image_path"] =  asset('backend/admin/images/equipment_management/equipment/');
            ...
            if ($events) {
                foreach ($events as $key => $event) {
                    ...
                    foreach (json_decode($images) as $yy => $image) {
                        ...
                        $dataArray['equipment'][$key]['image'][$yy] = asset('backend/admin/images/equipment_management/equipment/' . $image);
                    }
